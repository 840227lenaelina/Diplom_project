...
class SearchPage(BasePage):
    ...
    SEARCH_RESULTS_CONTAINER = (
        By.CSS_SELECTOR, ".search_results")  # Основной контейнер
    MOVIE_ITEMS = (
        By.CSS_SELECTOR, ".search_results .movie-item")  # Элементы фильмов
    MOVIE_TITLES = (By.CSS_SELECTOR, ".movie-item .title")  # Названия фильмов

    # Новые локаторы для блока "Скорее всего, вы ищете"
    MOST_LIKELY_RESULT = (
        By.CSS_SELECTOR, ".element.most_wanted")  # Блок "Скорее всего, вы ищете"
    MOST_LIKELY_TITLE = (
        By.CSS_SELECTOR, ".element.most_wanted .name a")  # Название фильма в основном результате
    SEARCH_RESULTS_LIST = (
        By.CSS_SELECTOR, ".search_results .element")  # Список всех результатов
    SEARCH_RESULT_TITLES = (
        By.CSS_SELECTOR, ".search_results .element .name a")  # Названия всех результатов

    # Локаторы для пустого поиска (обновлённые на основе HTML)
    RANDOM_MOVIE_BLOCK = (By.CSS_SELECTOR, ".randomMovie")  # Основной блок
    RANDOM_MOVIE_BUTTON = (
        By.CSS_SELECTOR, ".randomMovieButton .button")  # Конкретная кнопка
    NO_RESULTS_MESSAGE = (
        By.XPATH, "//*[contains(translate(text(), 'НИЧЕГО', 'ничего'), 'ничего не найдено')]")
    EMPTY_SEARCH_MESSAGE = (
        By.XPATH, "//*[contains(text(), 'Введите запрос') or contains(text(), 'заполните поле')]")
