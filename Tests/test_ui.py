import pytest
import allure
from KinoPages.main_page import MainPage
from KinoPages.search_page import SearchPage
...
class TestKinopoiskUI:
    ...
    @allure.title("Поиск фильма "
                 "по полному названию: {SEARCH_QUERIES['full_title']}")
    ...
    def test_search_by_partial_title(self, browser):
        ...
        with allure.step(f"Выполнить поиск по части \
                         названия: {SEARCH_QUERIES['partial_title']}"):
            main_page.search_for(SEARCH_QUERIES["partial_title"])

        with allure.step("Проверить результаты поиска"):
            # Проверяем, что появился блок "Скорее всего, вы ищете"
            assert search_page.is_most_likely_result_displayed(), "Блок "
            "с наиболее вероятным результатом не отображается"

            # Проверяем, что в этом блоке есть искомый фильм
            most_likely_title = search_page.get_most_likely_title()
            assert SEARCH_QUERIES[
                "partial_title"].lower() in most_likely_title.lower(), (
                f"Ожидалось найти '{
                    SEARCH_QUERIES['partial_title']}' в '{most_likely_title}'"
            )
    ...
    def test_search_nonexistent_movie(self, browser):
        ...
        with allure.step("Выполнить поиск несуществующего фильма"):
            main_page.search_for("Такого фильма нет")

        with allure.step("Проверить сообщение об отсутствии результатов"):
            assert search_page.is_no_results_found(), "Не отображается "
